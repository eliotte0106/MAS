...
import { Requests, RequestStatus, User } from '@/types'
...
export function RowActions({
    ...
    const handleConfirm = async () => {
        if( state.alertDialogReason === AlertDialogReasonEnum.DELETE) {
            ...
            setProgress(true)
            await fetch(buildUrl(`request/${request.id}`), {
                method: "DELETE"
            })
            setProgress(false)
    ...
    const handleInspectorAssign = async (inspector: User) => {
        try {
            ...
            setProgress(false)
            toast.success(`request assigned to ${inspector.fullName}`)
            router.refresh()
        } catch (error) {
            setProgress(false)
            console.log(error)
    ...
    const handleUnassign = async () => {
        try {
            ...
            setProgress(false);
            if (status === 200) {
                toast.success(`request marked as unassigned`);
                router.refresh();
            } else {
